...
// 获得单个
function* getConfigureOne(action) {
    try {
        const { jsonResult } = yield call(getConfigureListOne, action.formid);
        if (jsonResult.ok) {
            yield put({
                type: 'CONCONFIG/GET/ONE/SUCCESS',
                formdata: jsonResult.data,
            });
        }
    } catch (error) {
        console.log("getConfigureOne err");
        message.error(error);
    }
}
// 创建
...
function* watchConfigureOne() {
    yield takeLatest('CONCONFIG/GET/ONE', getConfigureOne)
}
